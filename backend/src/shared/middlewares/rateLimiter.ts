...
function calcularProfundidade(input: unknown, seen: WeakSet<object>): number {
    ...
    const objeto = input as object;
    if (seen.has(objeto)) return 0;
    seen.add(objeto);
    ...
    let max = 0;
    const registro = input as Record<string, unknown>;
...
export function LimiteProfundidadeJson(maxDepth: number = 7): RequestHandler {
    return (req, _res, next) => {
        try {
            const body = req.body;
            const depth = calcularProfundidade(body, new WeakSet<object>());
            if (depth > maxDepth) {
                return next(new AppError('Estrutura de dados excessivamente complexa.', 400, ErrorCodes.INVALID_JSON_STRUCTURE));
            }
            return next();
        } catch {
            return next(new AppError('Estrutura de dados excessivamente complexa.', 400, ErrorCodes.INVALID_JSON_STRUCTURE));
