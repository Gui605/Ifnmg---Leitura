...
async function enviarEmailComToken(email, token, acao) {
    ...
    if (!EMAIL_HOST || !EMAIL_USER || !EMAIL_PASS) {
        logger_1.logger.warn('SMTP não configurado. Simulando envio de e-mail.', {
    ...
    while (attempts < maxAttempts) {
        try {
            await send();
            logger_1.logger.info('E-mail transacional enviado', { evento: acao === 'verificacao' ? 'EMAIL_VERIFICATION_SENT' : 'PASSWORD_RECOVERY_EMAIL_SENT', acao, email });
            return;
        }
        catch (error) {
            attempts++;
            ...
            const isLogicalError = /invalid/i.test(msg) ||
                /no such user/i.test(msg) ||
                (respCode >= 400 && respCode < 500);
            if (isLogicalError || attempts >= maxAttempts) {
                logger_1.logger.error('Falha no envio de e-mail', { evento: acao === 'verificacao' ? 'EMAIL_VERIFICATION_FAILED' : 'PASSWORD_RECOVERY_EMAIL_FAILED', acao, email, attempts, error: msg, errorCode: ErrorCodes_1.ErrorCodes.EMAIL_SERVICE_UNAVAILABLE });
                throw new AppError_1.AppError('Serviço de e-mail indisponível. Tente novamente mais tarde.', 503, ErrorCodes_1.ErrorCodes.EMAIL_SERVICE_UNAVAILABLE);
            }
